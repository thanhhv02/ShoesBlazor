...
namespace PoPoy.Api.Services.NotificationService
{
    public class NotificationService : INotificationService
    {
        ...
        public async Task<bool> CreateNotificationAllAdmin(NotificationDto notiDto)
        {
            // gửi cho userId thuoc admin va staff
            var userIds = await userManager.GetUsersInRoleAsync(RoleName.Admin);
            foreach (var item in userIds)
            {
                Console.WriteLine(item.UserName);
            }
            var notis = new List<Notification>();
            foreach (var id in userIds.Select(p => p.Id))
            {
